...
<body>
	...
	<div>
		<form action="adminUserupload_editvideoinfo.action" method="post"
			enctype="multipart/form-data">
			<input id="vidId" type="hidden" name="uld"
				value="<s:property value="uploadVideoInfo.uld"/>" />
			...
			<table cellspacing="0" cellpadding="1" rules="all" bordercolor="gray"
				...
				<tr>
					...
					<td><input type="text" name="name"
						value="<s:property value="uploadVideoInfo.name"/>" /></td>
				...
				<tr>
					...
					<td><input type="text" name="duration"
						value="<s:property value="uploadVideoInfo.duration"/>" /></td>
					<td>影片简介</td>
					<td><textarea rows="3" cols="8" name="info" ><s:property value="uploadVideoInfo.info"/></textarea>
						

				</tr>
				<tr>				
					<td >上传视频地址</td>
					<td colspan="3"><input readonly="readonly" type="text" name=""
						id="couUrl111" value="<s:property value="uploadVideoInfo.url"/>"></td>
				</tr>
				<tr>
					
					<td>封面图片地址</td>
					<td colspan="3"><input readonly="readonly" name="" type="text"
						id="picture2" value="<s:property value="uploadVideoInfo.image"/>"></td>
				</tr>
				<tr>

					<td>状态</td>

					<s:if test="uploadVideoInfo.status==1">
						<td><select name="status">
								<option value="0">未审核</option>
								<option value="2">审核未通过</option>
						</select></td>
					</s:if>
					<s:if test="uploadVideoInfo.status==0">
						<td><select name="status"><option value="1">审核通过</option>

								<option value="2">审核未通过</option>
						</select></td>
					</s:if>
					<s:if test="uploadVideoInfo.status==2">
						<td><select name="status"><option value="1">审核通过</option>
								<option value="0">未审核</option>
