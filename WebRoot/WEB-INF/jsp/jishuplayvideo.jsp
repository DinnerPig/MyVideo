...
<title><s:property value="model.videoinfo.vidName" />:第<s:property value='model.couCount' />集</title>


<link rel="stylesheet" type="text/css" href="<%=basePath%>js/jsuggest.css">
...
function start_timer() {
	...
	if (timer2 == null)
		timer2 = window.setInterval("getDanmu()", 1000);
}
...
<body 
	...
	<div class="main">
		<div class="top1" style="width: 1000px;">
			...
			<div
				...
				<div style="float: left;">
					<input style="width: 200px" id="search" /> <input type="button" value="搜索" onclick="sou()"/>
				</div>
				<span id="dengluhou"></span>
				<div style="float: right;height:40px;width:200px;">
					<c:if test="${not empty  sessionScope.existUser}">
						<a style="cursor: pointer;"><label
							id="label2">用户名：${sessionScope.existUser.uName} </label></a>
					</c:if>
					<c:if test="${empty sessionScope.existUser}">
						<label id="check">

							<div id="xianshi" style="float:right;width: 300px; text-align:right;">
		...
		<div style="width: 100%;height: 500px;background-color:black">
			<div id="shipin" class="middle1" style="float: left;">
				<video id="mysp" width=100% height="490px" poster="img/c3.jpg"
					controls="controls">
				  <source src="<%=basePath%><s:property value='model.couUrl' />"
		...
		<div class="play1">
			...
			<a>点击数(<s:property value='model.couClick' />)</a>
				<a href="<%=basePath%>videocount_downloadFile.action?fileName=<s:property value='model.couUrl' />">下载</a>&nbsp;
			</div>
		</div>
		
		<input id="couId"type="hidden" value="<s:property value='model.couId' />" />
			  <input id="uid"  type="hidden"  value="${sessionScope.existUser.uid}" /> 
		  <input id="uName"  type="hidden"  value="${sessionScope.existUser.uName}" /> 
		<input id="vidId" type="hidden" value="<s:property value="model.videoinfo.vidId"/>" /> 
		<hr />
			<div class="lianjie">
			<div id="container" style="float: left;">
				...
				<tr>
					<s:iterator var="rb" value="videolist" status="st" begin="3" end="4">
					...
					<s:iterator var="rb" value="videolist" status="st" begin="7" end="8">
			...
			<div
				...
				<table width="100%" border="0">
					<tr>
						<td colspan="2">
							<h2><s:property value="model.videoinfo.vidName" /></h2>
						</td>
					</tr>
					<tr>
						<td width="50px">类型:</td>
						<td><s:property value="model.videoinfo.maxtype.maxName" /></td>
					</tr>
					<tr>
						<td>标签:</td>
						<td><s:property value="model.videoinfo.mintype.minName" /></td>
					</tr>
					<tr>
						<td>简介:</td>
						<td><s:property value="model.videoinfo.vidjianjie" /></td>
					</tr>
				</table>
...
<div class="comment">
		...
		<input type="button" value="展开评论" id="but" onclick="showcomment(1)"style="text-align: right; font-size: 18px" />
		<div id="commentdiv" class="commentinfo"  style='text-align:center'>
			<center>
			<label id="lab"> </label>
		</center>
		</div>
